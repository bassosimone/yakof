...
-------
    ...
    >>> xy2 = bases.project_xyz_to_xy_using_sum(xyz)  # project back to R²
    >>> x2 = bases.project_xy_to_x_using_sum(xy2)     # project back to R¹
"""
...
def expand_x_to_xy(t: Tensor[X]) -> Tensor[XY]:
    ...
    return Tensor[XY](graph.expand_dims(t.node, axis=1))


def expand_x_to_xz(t: Tensor[X]) -> Tensor[XZ]:
    """Expand X tensor by adding Z dimension at position 1."""
    return Tensor[XZ](graph.expand_dims(t.node, axis=1))


def expand_y_to_xy(t: Tensor[Y]) -> Tensor[XY]:
    """Expand Y tensor by adding X dimension at position 0."""
    return Tensor[XY](graph.expand_dims(t.node, axis=0))


def expand_y_to_yz(t: Tensor[Y]) -> Tensor[YZ]:
    """Expand Y tensor by adding Z dimension at position 1."""
    return Tensor[YZ](graph.expand_dims(t.node, axis=1))


def expand_z_to_xz(t: Tensor[Z]) -> Tensor[XZ]:
    """Expand Z tensor by adding X dimension at position 0."""
    return Tensor[XZ](graph.expand_dims(t.node, axis=0))


def expand_z_to_yz(t: Tensor[Z]) -> Tensor[YZ]:
    """Expand Z tensor by adding Y dimension at position 0."""
    return Tensor[YZ](graph.expand_dims(t.node, axis=0))


# Expansions from R² to R³
def expand_xy_to_xyz(t: Tensor[XY]) -> Tensor[XYZ]:
    """Expand XY tensor by adding Z dimension at position 2."""
    return Tensor[XYZ](graph.expand_dims(t.node, axis=2))


def expand_xz_to_xyz(t: Tensor[XZ]) -> Tensor[XYZ]:
    """Expand XZ tensor by adding Y dimension at position 1."""
    return Tensor[XYZ](graph.expand_dims(t.node, axis=1))


def expand_yz_to_xyz(t: Tensor[YZ]) -> Tensor[XYZ]:
    """Expand YZ tensor by adding X dimension at position 0."""
    return Tensor[XYZ](graph.expand_dims(t.node, axis=0))


# Projections from R² to R¹
def project_xy_to_x_using_sum(t: Tensor[XY]) -> Tensor[X]:
    """Project XY tensor to X by summing Y dimension (axis 1)."""
    return Tensor[X](graph.reduce_sum(t.node, axis=1))


def project_xy_to_y_using_sum(t: Tensor[XY]) -> Tensor[Y]:
    """Project XY tensor to Y by summing X dimension (axis 0)."""
    return Tensor[Y](graph.reduce_sum(t.node, axis=0))


def project_xz_to_x_using_sum(t: Tensor[XZ]) -> Tensor[X]:
    """Project XZ tensor to X by summing Z dimension (axis 1)."""
    return Tensor[X](graph.reduce_sum(t.node, axis=1))


def project_xz_to_z_using_sum(t: Tensor[XZ]) -> Tensor[Z]:
    """Project XZ tensor to Z by summing X dimension (axis 0)."""
    return Tensor[Z](graph.reduce_sum(t.node, axis=0))


def project_yz_to_y_using_sum(t: Tensor[YZ]) -> Tensor[Y]:
    """Project YZ tensor to Y by summing Z dimension (axis 1)."""
    return Tensor[Y](graph.reduce_sum(t.node, axis=1))


def project_yz_to_z_using_sum(t: Tensor[YZ]) -> Tensor[Z]:
    """Project YZ tensor to Z by summing Y dimension (axis 0)."""
    return Tensor[Z](graph.reduce_sum(t.node, axis=0))


# Projections from R³ to R²
def project_xyz_to_xy_using_sum(t: Tensor[XYZ]) -> Tensor[XY]:
    """Project XYZ tensor to XY by summing Z dimension (axis 2)."""
    return Tensor[XY](graph.reduce_sum(t.node, axis=2))


def project_xyz_to_xz_using_sum(t: Tensor[XYZ]) -> Tensor[XZ]:
    """Project XYZ tensor to XZ by summing Y dimension (axis 1)."""
    return Tensor[XZ](graph.reduce_sum(t.node, axis=1))


def project_xyz_to_yz_using_sum(t: Tensor[XYZ]) -> Tensor[YZ]:
    """Project XYZ tensor to YZ by summing X dimension (axis 0)."""
    return Tensor[YZ](graph.reduce_sum(t.node, axis=0))
